...
describe('resolveMaterialFunctions — known authoring-gap regressions', () => {
  // A /Game-only WorkMF index. Bridge / Megascans plugin content lives under
  // /Bridge and is NOT crawled unless WorkMfContentRoots includes it.
  const gameOnlyIndex: WorkMfIndex = {
    schemaVersion: '1.0', kind: 'workmf-index', ueVersion: '5.7',
    functions: {
      '/Game/Functions/MF_Foo.MF_Foo': {
        assetPath: '/Game/Functions/MF_Foo.MF_Foo', displayName: 'MF_Foo',
  ...
  const mfcGraph = (mf?: string): MatGraph => ({
    schemaVersion: '1.0', ueVersion: '5.7', type: 'Material', name: 'main',
    nodes: [mf
      ? { id: 'mfc', type: 'MaterialFunctionCall', params: { MaterialFunction: mf } }
      : { id: 'mfc', type: 'MaterialFunctionCall' }],
    connections: [],
  });

  // Residual: a Bridge-plugin MFC asset path is not in a /Game-only index, so it
  // must warn "not in index" rather than silently resolving. Fix = recrawl WorkMF
  // with -WorkMfContentRoots "/Game,/Bridge" (see docs/WORKMF.md).
  it('warns "not in index" for a /Bridge asset path when only /Game is crawled', async () => {
    const resolved = await resolveMaterialFunctions(
      mfcGraph('/Bridge/MaterialFunctions/MF_Surface.MF_Surface'), '/irrelevant', new Set(),
      { workMfIndex: gameOnlyIndex },
    );
    expect(resolved.derivedPins['mfc']).toEqual({ inputs: [], outputs: [] });
    expect(resolved.warnings.some(w => /not in index/i.test(w))).toBe(true);
  });

  // Residual: a MaterialFunctionCall whose MaterialFunction is None on import is
  // preserved as a node with no params.MaterialFunction; the resolver then emits
  // the clean "params.MaterialFunction missing" diagnosis (never a literal "None").
  it('warns "params.MaterialFunction missing" when the MFC has no MaterialFunction', async () => {
    const resolved = await resolveMaterialFunctions(
      mfcGraph(), '/irrelevant', new Set(), { workMfIndex: gameOnlyIndex },
    );
    expect(resolved.warnings.some(w => /params\.MaterialFunction missing/i.test(w))).toBe(true);
    expect(resolved.derivedPins['mfc']).toEqual({ inputs: [], outputs: [] });
  });
});
