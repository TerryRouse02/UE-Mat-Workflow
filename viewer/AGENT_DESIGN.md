...
Apply flow：讀現圖 → 逐 op 套用（apply 期錯誤回 `{opIndex, applyError}`）→
`validateGraph`＋`materialStructureWarnings`＋MF 解析（驗證期錯誤回
`{opIndex: null, validateErrors}`）→ 0 error 才 atomic 落盤 → 回 `{ok, diff[]}`。
