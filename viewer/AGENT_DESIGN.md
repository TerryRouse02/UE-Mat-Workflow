...
│     ├─ tools.ts               # 25 個 tool 定義 + dispatch（探索/視窗情境/記憶/壓縮/檔案管理/剪貼簿/爬取提案/DB 修改提案/公網/爬取 log）
...
content-parts 的 `image_url` data URI（圖前文後）。入口是 `AgentChatRequest.images`
（≤3 張、png/jpeg/webp/gif、單張 ≤5MB 解碼、http 層全驗證後才進 loop；chat body
上限提至 32MB）；UI 在輸入框直接貼上→縮圖預覽→隨訊息送出。transcript 只存
`images: 數量`不存資料（provider 歷史裡保留完整 block 供後續輪參照；
estimateMessagesTokens 把 image block 記 ~1.6K 視覺 token 而非 base64 字數，
否則恢復會話會誤觸天花板）。模型不支援視覺時 4xx 錯誤會附「請換視覺模型」提示。
maybeCompact 丟舊輪時圖片隨之丟棄；摘要序列化以「[使用者附了一張圖片]」代替。
...
| `read_memory` | `{scope}` | session / longterm 記憶內容 | 路徑寫死無穿越面；**team 模式 longterm 按用戶分檔** `.agent-memory/users/<名>.md`（用戶名過 slug 白名單），本地模式維持共享 `longterm.md`——成員間記憶互不污染（2026-06-12） |
...
| `get_node_signature` | `{name: 字串或 ≤8 個名稱的陣列}` | 單名＝完整 DB entry；批次＝`{found:{名:entry…}, missing?:{名:提示}}`（部分查無不算錯） | 查無時回近似名建議；批次省往返 |
...
{ "op": "setPosition", "id": "...", "pos": {"x": 0, "y": 0}, "why": "..." }  // UE 空間座標；僅在使用者要求排版時設
...
**自主工作流（規則 17–19，2026-06-15）**：在工具紀律之外，明確引導「對齊 → 規劃 → 動手 →
收尾複查」——17 需求不足時先用一句話對齊**會實質改變結果**的關鍵規格（質感/金屬/透明/發光/
平台），給夠或「你決定」就採合理預設並列出假設（與規則 13 的建立/修改歸屬互補、不重複問）；
18 複雜任務動手前先給一句步驟計畫，長對話可寫進 session 記憶；19 宣布完成前自我複查（最終態
...
| `/api/agent/approve` | POST | 寫入審查（review 模式）OWNER 對 `approval_request` 的決定（approve｜reject＋可選 reason），解開 loop 等待中的承諾 | `sameOrigin`；**owner 自審**（`canTouchSession`，永不發給 admin）；只認在記憶體中串流的會話 |
