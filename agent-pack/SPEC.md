...
into `nodes-ue5.7.export.json` (`materialAttributes`); the exporter reads it and supports **every**
attribute. Regenerate it by re-running `tools/node-t3d-metadata/Invoke-NodeT3DMetadataMaintenance.ps1`
...
**dropped from the export with a warning** (not invented).
