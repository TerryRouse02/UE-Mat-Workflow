...
> （重）打包永遠不會蓋掉已提交的 Win64 二進位或 `.uplugin`。`local.config.json` 設好後 `-EngineRoot`
> 可省略（會自動讀取設定檔）。**更省事：** 不論 Mac 或 Windows，viewer 的 Config 分頁 → 環境檢查那一步
> 都有一顆 **編譯插件** 按鈕，會替當前作業系統跑同一份 `Package-Plugin.ps1`（Win64 `.dll` 或 Mac `.dylib`），
> 免開終端機；完成後 `plugin` 檢查即轉綠。
...
當 `plugin` 是紅色（macOS 第一次跑、或升級 UE 導致外掛 ABI 不符時最常見），這一步的 **編譯插件** 按鈕會
透過 `Package-Plugin.ps1`（RunUAT BuildPlugin）替當前作業系統建出二進位，免開終端機。它需要 `platform`＋
`engine` 為綠（即有效的 `EngineRoot`）；過程會在面板即時串流，成功後該檢查轉綠。**Windows** 上會直接寫進
git 追蹤的 `compiled/UEMatExportMetadata/`（並改寫已提交的 `.uplugin`），事後請記得 `git commit`；**macOS**
只動 gitignored 的 `Binaries/Mac`（已提交的 Win64 二進位是在暫存目錄建置，不會被動到）。
