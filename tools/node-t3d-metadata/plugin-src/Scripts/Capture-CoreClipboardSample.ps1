param(
    ...
    [string]$Out = "",
    [string]$PackageDir = "",
    [string]$TextureAsset = "/Game/Textures/T_Mask.T_Mask"
)
...
if ([string]::IsNullOrWhiteSpace($TextureAsset)) {
    throw "TextureAsset is required. Pass a Texture2D object path such as /Game/Textures/T_Mask.T_Mask."
}
if ([string]::IsNullOrWhiteSpace($Out)) {
    $Out = Join-Path $WorkflowRoot "viewer\tests\fixtures\ue-clipboard-core.t3d"
}
...
$CommandletLog = Join-Path $LogRoot "UEMatExportMetadata_CoreClipboard.log"
...
Write-Host "Core clipboard sample written to $Out"
Write-Host "Texture asset: $TextureAsset"
