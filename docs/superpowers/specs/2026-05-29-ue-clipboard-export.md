...
6. **MaterialFunctionCall** → emit with `functionRefProperty`:
   - `params.MaterialFunction` is an engine path (`/…`) → pass through (`MaterialFunction=MaterialFunction'"<path>"'`); built-in MFs paste resolved.
   - local `./x.matgraph.json` → emit `MaterialFunction=None` + warn
     "assign function asset for: <id>" (user pastes the MF island into a new Material Function
     asset first, then assigns it).
...
The graph view gains two buttons:
- **「導出到 UE」 / Export to UE** — builds T3D from the open graph + current layout, calls
  `navigator.clipboard.writeText(text)`, shows a toast `Copied N nodes — paste into UE Material
  Editor` and lists any `warnings`.
...
built-in MF; the latter is preview-limited but exports/pastes correctly. Reiterate that x/y are
never written (export synthesizes positions).
