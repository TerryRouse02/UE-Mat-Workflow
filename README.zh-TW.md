...
- **建圖與修改**——每次寫入前先驗證（不合法的圖永遠不落盤），修改走增量 patch
  （加減單一節點、改接線、換節點類型，不整檔重寫），變更節點在畫布上脈衝高亮，
  每一輪都可「還原」或「重新生成」。
- **看得到你看的東西**——agent 需要時可查詢你目前開啟的圖與選取的節點，「這個節點」直接聽得懂，
  但開啟中的檔案不會變成誤傷目標：說「建立一個材質」一律寫**新檔**（要覆寫既有檔需你明確同意）。
...
- **拿進 UE**——說一聲就把圖以可貼上的 T3D 複製到剪貼簿（與標題列導出按鈕同一條路徑），
  到 UE 材質編輯器按 Ctrl+V。
- **查資料**——可插拔搜尋後端（Tavily／Brave／SearXNG，免金鑰 DuckDuckGo 保底）＋
  SSRF 防護的網頁抓取（長頁可分段續讀），支援 http 代理；輸入框旁有 🌐 開關
  （默認開：回覆前自判要不要查網路佐證；關閉：完全不連網）。
- **長對話不斷線**——會話持久化（切換／重播／刪除）、兩層記憶、自動上下文壓縮、
  每輪 token 用量顯示、一鍵把對話匯出成 Markdown。
- **不跑題**——它只聊 UE 材質／shader／遊戲開發。無關訊息第一次提醒、第二次拒答、
  第三次直接關閉並刪除該會話。
...
預設情況下 viewer 只綁 `127.0.0.1`、沒有登入——單人使用完全不變。直接在瀏覽器
開啟（Config → 團隊 → 啟用團隊模式：同一張表單建好管理員、現場重綁監聽位址，
不用重啟）——或在 Docker／腳本部署時用 `BIND_HOST=0.0.0.0`——同一台伺服器就變成
團隊工作區：

- **登入＋角色** — 帳號密碼換 7 天 token（HttpOnly cookie）。區域網下由第一個
  訪客建立管理員;公網部署則用 `ADMIN_USERNAME`／`ADMIN_PASSWORD` 預先植入,別人
  搶不到。之後在 Config → 使用者管理 新增成員。
- **admin** 擁有完整功能（agent 對話、爬取、LLM key、使用者管理）；**user**
  可看共享圖、匯入匯出、節點解說，並能唯讀圍觀管理員一鍵「設為公告」的
  **公告 agent 會話**（即時串流給全員）。
- LLM API key 由伺服器統一持有，永遠不會送到任何瀏覽器。

Docker / docker-compose / Caddy / nginx 範例與完整部署指南見
[`deploy/`](./deploy/README.md)——正式環境堆疊內建自動 HTTPS、非 root 唯讀容器、
安全標頭與管理員預植;區域網堆疊（`docker-compose.lan.yml`）則是給可信內網的純
HTTP。UE 爬取仍需要裝了 Unreal 的工作站——在那台機器上直接跑團隊模式，或在工作站
產好索引後掛載進容器。
